...
import { createMutationTestResult } from '../../helpers/mock';
import { StorageError } from '../../helpers/StorageError';
import { OptimisticConcurrencyError } from '../../../src';
...
describe(MutationTestingResultMapper.name, () => {
  ...
  describe('insertOrReplace', () => {
    ...
    it('should create the blob from text using application/json content type', async () => {
      ...
      expect(blobMock.createBlockBlobFromText).calledWith('mutation-testing-report', 'project;version;core', JSON.stringify(result), { contentSettings: { contentType: 'application/json', contentEncoding: 'utf8' } });
    });
    ...
    it('should throw OptimisticConcurrencyError "BlobHasBeenModified" is thrown', async () => {
      ...
      await expect(sut.insertOrReplace({ moduleName: 'core', projectName: 'project', version: 'version' }, null)).rejectedWith(OptimisticConcurrencyError);
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('should return the result', async () => {
      ...
      const actual = await sut.findOne({ moduleName: 'core', projectName: 'project', version: 'version' });
