...
describe(BlobServiceAsPromised.name, () => {
  ...
  describe('createTableIfNotExists', () => {
    it('should pass through the call to azure', async () => {
      const result = sut.createContainerIfNotExists('some-blob-storage', { publicAccessLevel: 'container' });
      blobServiceMock.createContainerIfNotExists.callArg(2);
      await result;
      expect(result).instanceof(Promise);
      expect(blobServiceMock.createContainerIfNotExists).calledWith('some-blob-storage', { publicAccessLevel: 'container' });
  ...
  describe('createBlockBlobFromText', () => {
    it('should pass through the call to azure', async () => {
      const result = sut.createBlockBlobFromText('foo', 'bar', 'baz', { contentSettings: { contentEncoding: 'utf8'}});
      blobServiceMock.createBlockBlobFromText.callArg(4);
      await result;
      expect(result).instanceof(Promise);
      expect(blobServiceMock.createBlockBlobFromText).calledWith('foo', 'bar', 'baz', { contentSettings: { contentEncoding: 'utf8'}});
