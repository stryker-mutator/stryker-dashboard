...
const firstToken = null as unknown as TableService.TableContinuationToken & common.ContinuationToken;
...
async function deleteAllBlobs(containerName: string) {
  let waitThereIsMore = true;
  while (waitThereIsMore) {
    const result = await listBlobsSegmented(containerName, firstToken) as BlobService.ListBlobsResult;
