...
export class BadgeConfigurator extends BaseElement {
  ...
  render() {
    return html`
      <div>
        <h3 class="mb-2 text-white text-xl">Choose from the following styles:</h3>	
        <div class="mb-4">
          ${map(badgeStyles, (style) => this.#renderBadge(style))}
          <sme-button class="block mt-2" .small="${true}" type="primary" @click="${this.#copyToClipboard}">${this.isCopied ? 'Copied!' : 'Copy badge'}</sme-button>
        </div>
      </div>
      <div>
  ...
  #copyToClipboard() {
    ...
    navigator.clipboard.writeText(`[![Mutation testing badge](${finalBadgeUrl}?style=${this.selectedStyle}&url=${badgeUrl})](${reportUrl})`);
