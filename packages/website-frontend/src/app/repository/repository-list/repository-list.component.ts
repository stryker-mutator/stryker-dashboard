...
export class RepositoryListComponent {
  ...
  public display(repositoryComponent: RepositorySwitchComponent, modal: RepositoryModalComponent) {
    dismissError(modal.openToDisplayRepository(repositoryComponent.repo));
  }

  public enable(repositoryComponent: RepositorySwitchComponent, modal: RepositoryModalComponent) {
    dismissError(modal.openToEnableRepository(repositoryComponent.repo)).catch(err => {
      repositoryComponent.flipSwitch();
      reportError({ error: err });
    });
  }

  public disable(repositoryComponent: RepositorySwitchComponent, modal: RepositoryModalComponent) {
    modal.openToDisableRepository(repositoryComponent.repo)
