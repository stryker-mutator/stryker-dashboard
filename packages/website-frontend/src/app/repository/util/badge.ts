...
export const allBadgeStyles: readonly BadgeStyle[] = Object.freeze(['flat', 'flat-square', 'plastic', 'for-the-badge']);
...
export function badgeSrc(repo: Repository, style: BadgeStyle = 'flat'): string {
  ...
  return `${shieldsBaseUrl}/endpoint?style=${style}&url=${encodeURIComponent(badgeApiUrl)}`;
}

export function badgeExampleSrc(score: number, color: string, style: BadgeStyle) {
