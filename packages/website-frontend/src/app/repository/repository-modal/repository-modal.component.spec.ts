...
describe(RepositoryModalComponent.name, () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      imports: [CommonModule, NgbModule, FormsModule, RouterTestingModule],
      providers: [
  ...
  describe(
    RepositoryModalComponent.prototype.openToEnableRepository.name,
    () => {
      ...
      describe('api section', () => {
        it('should generate an api key', () => {
          expect(repositoryServiceStub.enableRepository).toHaveBeenCalledWith(
            repository.slug,
            true
          );
        });
      ...
      //       modal.querySelectorAll('.card-header button')
      //     );
      //     const badgeButton = buttons.find(
      //       (button) => button.textContent!.trim() === 'Badge'
      ...
      //     const expectedUrl =
      //       'https://img.shields.io/endpoint?style=flat&url=https%3A%2F%2Fbadge-api.stryker-mutator.io%2Fgithub.com%2FfooOrg%2FbarRepo%2Fmaster';
      //     expect(modal.querySelector('code')!.textContent).toContain(
      //       expectedUrl
      //     );
      //   });
      // });
  ...
  describe(
    RepositoryModalComponent.prototype.openToDisplayRepository.name,
    () => {
      let modal: HTMLElement;
      let repository: Repository;
